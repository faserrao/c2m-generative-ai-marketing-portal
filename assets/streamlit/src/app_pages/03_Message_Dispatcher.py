...
def display_product_info(card_info):
    ...
    This function takes a dictionary of product information and displays it in
    a formatted manner. The product name, campaign phrase, description,
    key features, and great for sections are extracted from the dictionary and
...
def convert_value(val):
    ...
    If the value is a non-empty list, try to convert the first item to an int, then to a float,
    and if neither of those work, convert it to a string. If it's an empty list, return None.
    If it's not a list, return the value unchanged.
    ...
    if isinstance(val, list):
        if not val:  # Check for empty list
            return None
        item = val[0]
        try:
        ...
        # If none of the above work, return as string
        return str(item)
...
def process_df(dataframe):
    """Process the received dataframe by sorting, converting values, and
    reordering columns.
    ...
    Returns:
        tuple: A tuple containing the processed dataframe and the lists of user attributes,
        attributes, metrics, and other columns.
    """
    ...
    # Group the columns into categories
...
def extract_content(generated_text):
    ...
    The function expects the generated text to follow a specific format.
    ...
    Args:
        generated_text (str): The content generated by AI.

    Returns:
        tuple: A tuple containing the subject, HTML body, and text body.
    """
    try:
